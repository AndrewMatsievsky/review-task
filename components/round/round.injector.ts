...
export class RoundInjector {
  ...
  constructor(
    @inject(delay(() => RoundState)) private roundState?: RoundState,
  ) {
  ...
  public getWinPatternInjections = (pattern: string) => {
    ...
    if (Object.keys(this[InjectionType.TO_PATTERN]).length !== 0) {
      const list = this[InjectionType.TO_PATTERN].filter(
        ({ to }: InjectionToPatternItem) => pattern === to,
      );
      ...
      if (list.length) {
        ...
        this[InjectionType.TO_PATTERN] = this[InjectionType.TO_PATTERN].filter(
          ({ to }: InjectionToPatternItem) => pattern !== to,
        );
      }
    }
  ...
  public addInjectiveFunctions = (
    ...
    Object.assign(
      this.injectiveFunctions[module],
      ...functions.map((fn: Function) => ({ [fn.name]: fn })),
    );
  };
